...
/* Funções de filtragem */
    ...
    btn_pesquisar.addEventListener('click', (evt) => {
        ...
        if(f_pesq.value != ''){
            const endpointpesq = `${sv}/pesquisarfornecedor/${tipo}/${f_pesq.value}`
            fetch(endpointpesq)
            .then(res => res.json())
            .then(res => {
                dadosGrid.innerHTML = ''
...
/* Funções para carregar a tabela */
    ...
    const criarLinha = (e) => {
        ...
        img_editar.setAttribute('title', 'Editar fornecedor')
        img_editar.addEventListener('click', (evt) => {
            const id = evt.target.parentNode.parentNode.firstChild.innerHTML
            modojanela = 'e'
            document.getElementById('tituloPopup').innerHTML = 'Editar Fornecedor'
            const endpoint1 = `${sv}/dadosfornecedor/${id}`
            fetch(endpoint1)
            .then(res => res.json())
            .then(res => {
            ...
            const endpoint2 = `${sv}/todoscontatosfornecedor/${id}`
            fetch(endpoint2)
            .then(res => res.json())
            .then(res => {
                dadosGrid_contatosFornecedoresAdd.innerHTML = ''
                res.forEach(c => {
                    addContForn(c.n_pessoa_contatofornecedor, c.s_nome_pessoa, c.n_contatofornecedor_contatofornecedor)
                })
            })
        })
        ...
        if(sessionStorage.getItem('n_tipopessoa_pessoa') === '3'){
            ...
            img_remover.addEventListener('click', (evt) => {
                if(confirm('Deseja excluir este fornecedor?')){
                    const id = evt.target.parentNode.parentNode.firstChild.innerHTML
                    const endpoint = `${sv}/deletarfornecedor/${id}`
                    fetch(endpoint)
                    .then(res => {
                        if(res.status == 200){
...
/* Funções de contato de fornecedor */
    const addContForn = (id, nome, idf) => {
        ...
        img_verFoneContForn.addEventListener('click', (evt) => {
            const id = evt.target.parentNode.parentNode.firstChild.innerHTML
            const endpoint = `${sv}/retornartelefones/${id}`
            fetch(endpoint)
            .then(res => res.json())
            .then(res => {
        ...
        img_removerContForn.addEventListener('click', (evt) => {
            if(modojanela == 'e'){
                if(idf != 0) {
                    img_removerContForn.setAttribute('data-idcontato', idf)
                }
                if(confirm('A ação será irreversível, deseja excluir este contato?')){
                    const objContat = evt.target.parentNode.parentNode
                    const idContat = evt.target.getAttribute('data-idcontato')
                    if(idContat){
                        const endpoint = `${sv}/deletarcontato/${idContat}` 
                        fetch(endpoint)
                        .then(res => {
                            if(res.status == 200){
                                objContat.remove()
...
/* Eventos de botões internos e externos do pop-up principal e conexão de APIs */
    ...
    btn_gravarPopup.addEventListener('click', (evt) => {
        ...
        let a_contat = []
        contat.forEach(c => {
            a_contat.push(c.firstChild.innerHTML)
        }) 
