...
const renovarToken = (ts, dt) => {
    ...
    const endpoint = `${sv}/renovartoken`
...
const verificarToken = () => {
    ...
    .then(r => {
        ...
        if(validade && tsatual <= validade){
            ...
            const endpointLogoff = `${sv}/deletartoken/${token}`
...
/* Funções de filtragem */
    ...
    btn_pesquisar.addEventListener('click', (evt) => {
        ...
        if(f_pesq.value != ''){
            const endpointpesq = `${sv}/pesquisarproduto/${tipo}/${f_pesq.value}`
            fetch(endpointpesq)
            .then(res => res.json())
            .then(res => {
                dadosGrid.innerHTML = ''
...
/* Funções para carregar a tabela */
    ...
    const criarLinha = (e) => {
        ...
        img_status.setAttribute('data-idproduto', e.n_cod_produto)
        ...
        img_status.addEventListener('click', (evt) => {
            ...
            if(evt.target.getAttribute('src') == '../../shared/img/on.svg'){
                const endpoint_mudarStatus = `${sv}/mudarstatusproduto/${idproduto}/I`
                ...
                const endpoint_mudarStatus = `${sv}/mudarstatusproduto/${idproduto}/A`
                fetch(endpoint_mudarStatus)
                .then(res => {
                    if(res.status == 200){
        ...
        img_editar.addEventListener('click', (evt) => {
            ...
            const endpoint = `${sv}/dadosproduto/${id}`
            fetch(endpoint)
            .then(res => res.json())
            .then(res => {
                btn_gravarPopup.setAttribute('data-idproduto', id)
                f_codprod.value = res[0].n_cod_produto
                f_descprod.value = res[0].s_desc_produto
                f_qtdeprod.value = res[0].n_qtde_produto
                f_tipoprod.value = res[0].n_tipoproduto_produto
                f_fornprod.value = res[0].n_fornecedor_produto
        ...
        img_movimentar.setAttribute('src', '../../shared/img/mov.svg')
        ...
        img_movimentar.addEventListener('click', (evt) => {
            ...
            if(l.childNodes[3].innerHTML == 'A'){
                ...
                const config = {
                titulo: "Alerta",
                texto: "Produtos inativos (status) não podem ser modificados.",
                cor: "#008",
                tipo: "ok",
                ok: () => {},
                sim: () => {},
        ...
        if(sessionStorage.getItem('n_tipopessoa_pessoa') === '3'){
            ...
            img_remover.addEventListener('click', (evt) => {
                if(confirm('Deseja excluir este produto?')){
                    const id = evt.target.parentNode.parentNode.firstChild.innerHTML
                    const endpoint = `${sv}/deletarproduto/${id}`
                    fetch(endpoint)
                    .then(res => {
                        if(res.status == 200){
...
/* Eventos de botões internos e externos do pop-up principal e conexão de APIs */
    ...
    btn_gravarmov.addEventListener('click', (evt) => {
        ...
        const cabecalho= {
            ...
            body: JSON.stringify(qtde)
        }

        const endpoint = `${sv}/editarmovproduto`
        fetch(endpoint, cabecalho)
        .then(res => {
            if(res.status == 200){
                const config = {
                    titulo: "OK",
                    texto: "Movimentação efetuada com sucesso!",
                    cor: "#008",
                    tipo: "ok",
                    ok: () => {},
                    sim: () => {},
    ...
    btn_addqtde.addEventListener('click', (evt) => {
        ...
        let qtdeadd = Math.abs(parseInt(f_qtdeprodregmov.value))
        qtdeatual += qtdeadd
    ...
    btn_removeqtde.addEventListener('click', (evt) => {
        let qtdeatual = parseInt(f_qtdeprodmov.value)
        let qtderem = Math.abs(parseInt(f_qtdeprodregmov.value))
        if(qtderem <= qtdeatual){
            qtdeatual -= qtderem
        }else{
            const config = {
            titulo: "Alerta",
            texto: "Valores não podem ser maiores que o esperado.",
            cor: "#008",
            tipo: "ok",
            ok: () => {},
            sim: () => {},
    ...
    btn_gravarPopup.addEventListener('click', (evt) => {
        ...
        if(modojanela == 'n'){
            ...
            endpoint = `${sv}/editarproduto/${evt.target.dataset.idproduto}`
        }
        fetch(endpoint, cabecalho)
        .then(res => {
            if(res.status == 200){
