...
app.get('/deletarfornecedor/:id', (req, res) => { 
  const parametro = req.params.id;
  const query = 'DELETE FROM fornecedor WHERE n_fornecedor_fornecedor = ?';
...
app.get('/deletarproduto/:id', (req, res) => { 
  const parametro = req.params.id;
  const query = 'DELETE FROM produto WHERE n_cod_produto = ?';
...
app.get('/userlogin/:username', (req, res) => {
  const email = req.params.username;
  const query = `SELECT n_pessoa_pessoa, s_nome_pessoa, n_tipopessoa_pessoa FROM pessoa WHERE s_email_pessoa = ?`;
...
app.get('/sessaotoken/:id', (req, res) => { 
  const parametro = req.params.id;
  const query = `SELECT n_token_token, s_desc_token FROM token WHERE n_pessoa_token = ?`;
...
app.get('/verificartoken/:token', (req, res) => { 
  const parametro = req.params.token;
  const query = `SELECT s_validade_token FROM token WHERE s_desc_token = ?`;
...
app.put('/renovartoken', (req, res) => {
  const { s_validade_token, s_desc_token } = req.body;
  const query = `UPDATE token SET s_validade_token = ? WHERE s_desc_token = ?`;
  const values = [s_validade_token, s_desc_token];
...
app.get('/deletartoken/:token', (req, res) => { 
  const parametro = req.params.token;
  const query = 'DELETE FROM token WHERE s_desc_token = ?';

  connection.query(query, [parametro], (error, results) => {
    if(error){
      console.error('Erro ao executar a query:', error);
      return res.status(500).send('Erro ao excluir token');
    }
    res.json(results);
  });
});
