...
app.get('/login/:username/:password', (req, res) => { 
  ...
  if(senha != '$empty'){
    query1 = 'SELECT n_pessoa_pessoa, n_primacess_pessoa FROM pessoa WHERE s_email_pessoa = ? AND s_senha_pessoa = ?';
    values.push(senha);        
  }else{
    query1 = 'SELECT n_pessoa_pessoa, n_primacess_pessoa FROM pessoa WHERE s_email_pessoa = ?';
  }
