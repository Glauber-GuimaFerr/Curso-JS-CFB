...
const criarLinha = (e) => {
    ...
    img_status.setAttribute('data-idfornecedor', e.n_pessoa_pessoa)
    ...
    img_editar.addEventListener('click', (evt) => {
        ...
        .then(res => {
            ...
            f_tipoFornec.value = res[0].n_tipopessoa_pessoa
...
const endpoint_tiposFornec = `${sv}/tiposfornecedor`
fetch(endpoint_tiposFornec)
.then(res => res.json())
.then(res => {
    f_tipoFornec.innerHTML = ''
    res.forEach(e => {
        const opt = document.createElement('option')
        opt.setAttribute('value', e.n_tipopessoa_tipopessoa)
        opt.innerHTML = e.s_desc_tipopessoa
        f_tipoFornec.appendChild(opt)
    })
})
...
btn_gravarPopup.addEventListener('click', (evt) => {
    ...
    const dados = {
        ...
        n_tipopessoa_pessoa: f_tipoFornec.value,
    ...
    let endpoint_novoXeditarFornec = null
    if(modojanela == 'n'){
        endpoint_novoXeditarFornec = `${sv}/novofornecedor` 
    }else{
        endpoint_novoXeditarFornec = `${sv}/editarfornecedor/${evt.target.dataset.idfornecedor}`
    }
    fetch(endpoint_novoXeditarFornec, cabecalho)
    .then(res => {
        if(res.status == 200){
            if(modojanela == 'n'){
                const config = {
