...
app.put('/editarcolab/:id', (req, res) => {
  ...
  connection.query(query1, values, (error, results) => {
    ...
    if(Array.isArray(numtelefones) && numtelefones.length > 0){
      ...
      const query2 = `
        INSERT INTO telefone (n_telefone_telefone, n_usuario_telefone, s_numero_telefone) VALUES ?`;
...
app.get('/mudarstatuscolab/:id/:status', (req, res) => { 
  ...
  const query = 'UPDATE usuario SET c_status_usuario = ? WHERE n_usuario_usuario = ?';
...
app.get('/deletarcolab/:id', (req, res) => { 
  const parametro = req.params.id;
  const query = 'DELETE FROM usuario WHERE n_usuario_usuario = ?';
...
app.get('/pesquisacolab/:tipo/:valor', (req, res) => { 
  ...
  let query = '';
  if(tipo == 'id'){
    query = 'SELECT * FROM usuario WHERE n_usuario_usuario = ?';    
  }else{
    valor = `%${req.params.valor}%`;
    query = 'SELECT * FROM usuario WHERE s_nome_usuario LIKE ?';
  }
