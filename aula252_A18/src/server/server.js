...
app.get('/login/:username/:password', (req, res) => { 
  ...
  const query1 = 'SELECT n_pessoa_pessoa, n_primacess_pessoa, s_nome_pessoa FROM pessoa WHERE s_email_pessoa = ? AND s_senha_pessoa = ?';
  ...
  connection.query(query1, values, (error, results) => {
    ...
    if(user.n_primacess_pessoa == 0){
      ...
      connection.query(query2, updateValues, (error, results2) => {
        if(error){
